...
},
{
  ...
  schema:{"events":[{"col":"visit_id","type":"TEXT","note":""},{"col":"cookie_id","type":"TEXT","note":""},{"col":"page_id","type":"INTEGER","note":"nullable"},{"col":"event_type","type":"INTEGER","note":"1=view,2=cart,3=purchase"},{"col":"sequence_number","type":"INTEGER","note":""},{"col":"event_time","type":"TIMESTAMP","note":""}],"users":[{"col":"user_id","type":"INTEGER","note":"pk"},{"col":"cookie_id","type":"TEXT","note":""},{"col":"start_date","type":"DATE","note":""}]},
  seed:"CREATE TABLE users(user_id INTEGER, cookie_id TEXT, start_date TEXT);\nINSERT INTO users VALUES\n(1,'c1','2020-01-01'),(2,'c2','2020-01-02'),(3,'c3','2020-01-03'),(4,'c4','2020-01-04');\n\nCREATE TABLE events(visit_id TEXT, cookie_id TEXT, page_id INTEGER, event_type INTEGER, sequence_number INTEGER, event_time TEXT);\nINSERT INTO events VALUES\n('v101','c1',1,1,1,'2020-02-01 10:00:00'),('v101','c1',3,2,2,'2020-02-01 10:01:00'),('v101','c1',3,3,3,'2020-02-01 10:02:00'),('v101','c1',9,3,4,'2020-02-01 10:03:00'),\n('v102','c1',1,1,1,'2020-02-05 09:00:00'),('v102','c1',5,3,2,'2020-02-05 09:02:00'),\n('v103','c2',1,1,1,'2020-02-02 12:00:00'),('v103','c2',4,3,2,'2020-02-02 12:01:00'),('v103','c2',6,3,3,'2020-02-02 12:03:00'),\n('v104','c3',1,1,1,'2020-02-03 14:00:00'),('v104','c3',8,3,2,'2020-02-03 14:02:00'),\n('v105','c3',1,1,1,'2020-02-06 15:00:00'),('v105','c3',2,2,2,'2020-02-06 15:01:00'),\n('v106','c4',1,1,1,'2020-02-04 11:00:00'),('v106','c4',7,2,2,'2020-02-04 11:01:00');\n",
  example:{"cols":["user_id","visits_with_purchase","total_purchases","avg_purchases_per_visit"],"rows":[[1,2,3,1.5],[2,1,2,2],[3,1,1,1]]},
  hint:"The <code>events</code> table has <code>cookie_id</code>, not <code>user_id</code> — join it to <code>users</code> on <code>cookie_id</code>. Keep only <code>event_type = 3</code>, count purchases per <code>(user_id, visit_id)</code> in a subquery, then per user use COUNT for visits, SUM for total, and AVG for the average.",
  testCases:[
    { name:"3 purchasing users returned (non-purchasers excluded)", seed:null, check:(r)=>r.length===3 },
    { name:"User 1: 2 visits, 3 purchases, avg 1.5", seed:null, check:(r)=>{const x=r.find(y=>Number(y.user_id)===1);return !!x&&Number(x.visits_with_purchase)===2&&Number(x.total_purchases)===3&&Number(x.avg_purchases_per_visit)===1.5;} },
    { name:"User 2: 1 visit, 2 purchases, avg 2", seed:null, check:(r)=>{const x=r.find(y=>Number(y.user_id)===2);return !!x&&Number(x.visits_with_purchase)===1&&Number(x.total_purchases)===2&&Number(x.avg_purchases_per_visit)===2;} },
    { name:"User 3 non-purchasing visit excluded (1 visit, 1 purchase)", seed:null, check:(r)=>{const x=r.find(y=>Number(y.user_id)===3);return !!x&&Number(x.visits_with_purchase)===1&&Number(x.total_purchases)===1;} },
    { name:"User 4 (no purchase) not present", seed:null, check:(r)=>!r.find(y=>Number(y.user_id)===4) },
  ]
},
{
